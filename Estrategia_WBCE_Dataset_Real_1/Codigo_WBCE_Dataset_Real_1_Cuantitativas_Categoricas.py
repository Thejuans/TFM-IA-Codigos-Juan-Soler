...
# Backend no interactivo para evitar errores de Tkinter al guardar figuras
# cuando se usa paralelización con joblib en Windows/VS Code.
import matplotlib
matplotlib.use("Agg")
...
def format_scenario_label_for_plot(scenario_name):
    ...
    return SCENARIO_DISPLAY_NAMES.get(
        scenario_text,
        scenario_text.replace("_", " ").title(),
    )
...
def split_metric_name(metric):
    ...
    - métrica base: f1, balanced_accuracy, pr_auc, etc.;
...
# ============================================================
# 0.1. DATASET REAL 1 · VARIABLES CUANTITATIVAS + CATEGÓRICAS
# ============================================================
...
# Para evitar que identificadores o columnas de texto libre generen demasiadas variables dummy,
# solo se incluyen como categóricas las columnas no numéricas con cardinalidad moderada.
MAX_CATEGORICAL_UNIQUE_VALUES = 30

# Referencias explícitas para variables categóricas multiclase.
# La categoría indicada se omite en el One-Hot y actúa como grupo base.
# Si una variable multiclase no está aquí, se usa como referencia la primera
# categoría en orden alfabético para mantener un criterio fijo y reproducible.
REFERENCE_CATEGORIES = {
    "LCA_REUS": "High Satisfaction",
}

PROJECT_DIR = Path(__file__).resolve().parent
DATA_FILE_PATH = PROJECT_DIR / DATA_FILE_NAME

# Se mantiene la estructura de carpetas original del proyecto.
# Solo se acortan algunos nombres de archivos internos para evitar rutas demasiado largas.
OUTPUT_DIR = PROJECT_DIR / "outputs_dataset_real_1_cuantitativas_categoricas"
...
def get_short_scenario_name(scenario_name):
    """Nombre corto, pero legible, para archivos auxiliares."""
    if scenario_name == REAL_SCENARIO_NAME:
        return "real1_cuant_categ"
    return str(scenario_name).replace(" ", "_")[:24]

# ============================================================
# 0.2. ORGANIZACIÓN DE FIGURAS
# ============================================================
...
def get_scenario_figure_path(scenario_name, filename):
    """
    Devuelve la ruta organizada de una figura propia del escenario.

    Estructura final simplificada:
    outputs_dataset_real_1_cuantitativas_categoricas/figures/<escenario>/<archivo.png>
...
def get_global_figure_path(filename):
    ...
    Estructura final simplificada:
    outputs_dataset_real_1_cuantitativas_categoricas/figures/comparativas_globales/<archivo.png>
...
RAW_PREDICTIONS_PATH = OUTPUT_DIR / "predictions_raw_multiseed.csv"
...
RUN_SIGNATURE = "real1_cuant_categ_grid30_v4_clean_analysis_short_filenames"
...
# En este trabajo se interpreta la clase 1 como la clase positiva/caso de interés
# y la clase 0 como la clase negativa/no caso.
SCENARIOS = {
    REAL_SCENARIO_NAME: {
        "dataset": DATA_FILE_NAME,
        "target": TARGET_COL,
        "type": "real_dataset_quantitative_categorical_binary_and_one_hot_reference",
        "description": "Dataset real 1 usando variables cuantitativas y categóricas: binarias a 0/1 y multiclase mediante One-Hot con categoría de referencia.",
    }
}

pd.DataFrame([
    {
        "scenario": REAL_SCENARIO_NAME,
        "dataset": DATA_FILE_NAME,
        "target": TARGET_COL,
        "features_type": "quantitative_categorical_binary_and_one_hot_reference",
        "seeds": len(SEEDS),
        "n_c_values": len(C_VALUES),
        "grid_size": len(C_VALUES) * len(C_VALUES),
        "split_train_pct": 0.72,
        "split_val_pct": 0.18,
        "split_test_pct": 0.10,
        "missing_policy": "drop_rows_with_missing_in_selected_quantitative_or_categorical_features",
        "imputation": False,
        "model_max_iter": MODEL_MAX_ITER,
        "solver": "lbfgs_default",
    }
]).to_csv(SCENARIOS_CONFIG_PATH, index=False)

# Caché simple para no leer y analizar el Excel continuamente.
_CLEAN_DF_CACHE = None
_FEATURE_COLS_CACHE = None
_RAW_DF_CACHE = None
_EXCLUDED_COLS_CACHE = None
_CLEANING_INFO_CACHE = None

# ============================================================
# 2. ANÁLISIS, LIMPIEZA Y PARTICIONES DEL DATASET REAL
# ============================================================

def get_feature_cols_from_df(df):
    """
    Devuelve las variables predictoras del dataset real.

    A diferencia de los datasets sintéticos, aquí se mantienen los nombres
    originales de las variables cuantitativas, por lo que no se filtra por
    prefijo x_. Se excluyen únicamente la variable objetivo, split y sample_id.
    """
...
def _clean_column_names(df):
    """
    Limpia los nombres de columnas quitando espacios iniciales/finales.
    No cambia el significado de las variables.
    """
    df = df.copy()
...
def _coerce_binary_target(series):
    """
    Convierte la variable objetivo a 0/1 entero.
    """
    s = series.copy()

    if pd.api.types.is_numeric_dtype(s):
        non_null_values = sorted(pd.Series(s.dropna().unique()).tolist())
        if set(non_null_values).issubset({0, 1, 0.0, 1.0}):
            return s.astype(int)
    ...
    if mapped.isna().any():
        ...
        raise ValueError(
            f"No se puede convertir la variable objetivo {TARGET_COL} a 0/1. "
...
def save_dataset_analysis_figures(clean_df, feature_cols, class_distribution_df):
    """
    Guarda figuras simples de análisis exploratorio del dataset real.

    La representación se realiza sobre las variables finales ya codificadas:
    - variables cuantitativas originales;
    - variables categóricas binarias transformadas a 0/1;
    - variables categóricas multiclase codificadas mediante One-Hot.
    """
    # Distribución de clases.
    plt.figure(figsize=(6.5, 4.8))
    x_labels = [f"Clase {int(c)}" for c in class_distribution_df["class"]]
    y_values = class_distribution_df["count"].values
    max_count = float(np.nanmax(y_values)) if len(y_values) > 0 else 1.0
    label_offset = max(2.0, 0.025 * max_count)

    bars = plt.bar(x_labels, y_values, color=["#1f77b4", "#d62728"], edgecolor="white")

    for bar, count, prop in zip(bars, class_distribution_df["count"], class_distribution_df["proportion"]):
        plt.text(
    ...
    plt.title("Dataset real 1 · Distribución de clases", pad=18)
    plt.ylabel("Número de muestras")
    plt.tight_layout(rect=[0, 0, 1, 0.93])
    plt.savefig(get_scenario_figure_path(REAL_SCENARIO_NAME, f"{REAL_SCENARIO_NAME}_target_distribution.png"), dpi=180)
    plt.close()

    # Se elimina la figura de correlación para mantener el análisis más ligero.


def print_dataset_analysis_terminal_summary(raw_df, clean_df, feature_cols, class_distribution_df, excluded_cols, cleaning_info):
    """
    Muestra por terminal un resumen exploratorio breve y solo con la información clave.

    Esta función se llama una única vez desde main(), antes de iniciar TRAIN.
    No se llama desde los procesos paralelos de joblib para evitar mensajes repetidos.
    """
    if cleaning_info is None:
        cleaning_info = {}

    quantitative_cols = list(cleaning_info.get("quantitative_feature_cols", []))
    categorical_cols = list(cleaning_info.get("categorical_feature_cols", []))
    binary_categorical_cols = list(cleaning_info.get("binary_categorical_cols", []))
    multiclass_categorical_cols = list(cleaning_info.get("multiclass_categorical_cols", []))
    encoded_cols = list(cleaning_info.get("encoded_feature_cols", feature_cols))

    print("\n" + "=" * 80)
    print("ANÁLISIS EXPLORATORIO BREVE · DATASET REAL 1")
    print("=" * 80)
    print(f"Archivo: {DATA_FILE_NAME} | Target: {TARGET_COL} | Escenario: {REAL_SCENARIO_NAME}")
    print("")

    print("[1] Limpieza y tamaño")
    print(
        f"  - Filas: {cleaning_info.get('original_rows_excel', len(raw_df))} originales -> "
        f"{len(clean_df)} finales"
    )
    print(
        f"  - Eliminadas: duplicados={cleaning_info.get('duplicates_removed', 0)}, "
        f"target ausente={cleaning_info.get('rows_removed_missing_target', 0)}, "
        f"missing en variables seleccionadas={cleaning_info.get('rows_removed_missing_selected_features', 0)}"
    )
    print("  - Imputación aplicada: NO")
    print("")

    print("[2] Distribución de clases")
    for _, row in class_distribution_df.iterrows():
        print(
            f"  - Clase {int(row['class'])}: {int(row['count'])} muestras "
            f"({row['proportion'] * 100:.2f}%)"
        )
    print("")

    print("[3] Variables usadas")
    print(f"  - Cuantitativas originales: {len(quantitative_cols)}")
    print(f"  - Categóricas originales: {len(categorical_cols)}")
    print(f"  - Categóricas binarias transformadas a 0/1: {len(binary_categorical_cols)}")
    print(f"  - Categóricas multiclase con One-Hot: {len(multiclass_categorical_cols)}")
    print(f"  - Variables predictoras finales tras codificación: {len(encoded_cols)}")
    print("")

    print("[4] Top 5 variables más correlacionadas con el target")
    corr_terminal_rows = []
    for col in encoded_cols:
        if clean_df[col].nunique(dropna=True) > 1:
            corr_value = clean_df[[col, TARGET_COL]].corr().iloc[0, 1]
        else:
            corr_value = np.nan
        corr_terminal_rows.append({"feature": col, "corr": corr_value})

    corr_terminal_df = pd.DataFrame(corr_terminal_rows).dropna()
    if corr_terminal_df.empty:
        print("  - No hay correlaciones calculables.")
    else:
        corr_terminal_df = corr_terminal_df.assign(abs_corr=corr_terminal_df["corr"].abs())
        corr_terminal_df = corr_terminal_df.sort_values("abs_corr", ascending=False).head(5)
        for _, row in corr_terminal_df.iterrows():
            print(f"  - {row['feature']}: corr={row['corr']:.4f}")
    print("")

    print("[5] Archivos del análisis exploratorio")
    print(f"  - Tablas/resumen: {ANALYSIS_DIR}")
    print(f"  - Figuras: {FIGURES_DIR}")
    print("=" * 80 + "\n")

def save_dataset_analysis_outputs(raw_df, clean_df, feature_cols, excluded_cols, n_duplicates_removed, n_target_na_removed, cleaning_info=None):
    """
    Guarda un análisis inicial del dataset real antes del entrenamiento.

    Esta versión permite justificar en la memoria:
    - tamaño del dataset;
    - distribución de clases;
    - variables cuantitativas usadas;
    - variables categóricas incorporadas;
    - variables categóricas binarias transformadas a 0/1;
    - variables categóricas multiclase codificadas mediante One-Hot con categoría de referencia;
    - limpieza aplicada sin imputación.
    """
    class_counts = clean_df[TARGET_COL].value_counts().sort_index()
    class_props = clean_df[TARGET_COL].value_counts(normalize=True).sort_index()
    ...
    class_distribution_df.to_csv(ANALYSIS_DIR / "target_class_distribution.csv", index=False)
    ...
    binary_categorical_cols = list(cleaning_info.get("binary_categorical_cols", []))
    multiclass_categorical_cols = list(cleaning_info.get("multiclass_categorical_cols", []))
    binary_mapping_rows = list(cleaning_info.get("binary_mapping_rows", []))
    one_hot_reference_rows = list(cleaning_info.get("one_hot_reference_rows", []))
    encoded_cols = list(cleaning_info.get("encoded_feature_cols", feature_cols))
    constant_encoded_cols = list(cleaning_info.get("constant_encoded_cols", []))

    cleaning_summary_df = pd.DataFrame([
        {"step": "filas_originales_excel", "value": cleaning_info.get("original_rows_excel", len(raw_df))},
        {"step": "columnas_originales_excel", "value": cleaning_info.get("original_columns_excel", raw_df.shape[1])},
        {"step": "columnas_unnamed_eliminadas", "value": cleaning_info.get("unnamed_columns_removed", 0)},
        {"step": "columnas_completamente_vacias_eliminadas", "value": cleaning_info.get("empty_columns_removed", 0)},
        {"step": "filas_completamente_vacias_eliminadas", "value": cleaning_info.get("empty_rows_removed", 0)},
        {"step": "duplicados_exactos_eliminados", "value": n_duplicates_removed},
        {"step": "filas_eliminadas_por_target_ausente", "value": n_target_na_removed},
        {"step": "filas_con_missing_en_variables_seleccionadas_antes_dropna", "value": cleaning_info.get("rows_with_missing_in_selected_features_before_dropna", 0)},
        {"step": "missing_total_en_variables_seleccionadas_antes_dropna", "value": cleaning_info.get("missing_values_in_selected_features_before_dropna", 0)},
        {"step": "filas_eliminadas_por_missing_en_variables_seleccionadas", "value": cleaning_info.get("rows_removed_missing_selected_features", 0)},
        {"step": "filas_finales_disponibles", "value": len(clean_df)},
        {"step": "variables_cuantitativas_originales_seleccionadas", "value": len(quantitative_cols)},
        {"step": "variables_categoricas_originales_seleccionadas", "value": len(categorical_cols)},
        {"step": "variables_categoricas_binarias_transformadas_0_1", "value": len(binary_categorical_cols)},
        {"step": "variables_categoricas_multiclase_one_hot_con_referencia", "value": len(multiclass_categorical_cols)},
        {"step": "variables_predictoras_finales_tras_codificacion", "value": len(encoded_cols)},
        {"step": "columnas_constantes_eliminadas_tras_codificacion", "value": len(constant_encoded_cols)},
        {"step": "max_categorical_unique_values", "value": cleaning_info.get("max_categorical_unique_values", MAX_CATEGORICAL_UNIQUE_VALUES)},
    ])
    cleaning_summary_df.to_csv(ANALYSIS_DIR / "cleaning_summary.csv", index=False)

    pd.DataFrame({
        "feature": quantitative_cols,
        "dtype": [str(raw_df[col].dtype) for col in quantitative_cols],
        "missing_count_before_dropna": [int(raw_df[col].isna().sum()) for col in quantitative_cols],
        "missing_pct_before_dropna": [float(raw_df[col].isna().mean()) for col in quantitative_cols],
        "n_unique": [int(raw_df[col].nunique(dropna=True)) for col in quantitative_cols],
    }).to_csv(ANALYSIS_DIR / "selected_quantitative_features.csv", index=False)

    pd.DataFrame({
        "feature": categorical_cols,
        "encoding": [
            "binary_0_1" if col in binary_categorical_cols else "one_hot_reference"
            for col in categorical_cols
        ],
        "dtype": [str(raw_df[col].dtype) for col in categorical_cols],
        "missing_count_before_dropna": [int(raw_df[col].isna().sum()) for col in categorical_cols],
        "missing_pct_before_dropna": [float(raw_df[col].isna().mean()) for col in categorical_cols],
        "n_unique": [int(raw_df[col].nunique(dropna=True)) for col in categorical_cols],
    }).to_csv(ANALYSIS_DIR / "selected_categorical_features.csv", index=False)

    pd.DataFrame(binary_mapping_rows).to_csv(
        ANALYSIS_DIR / "binary_categorical_mapping_0_1.csv",
        index=False,
    )

    pd.DataFrame(one_hot_reference_rows).to_csv(
        ANALYSIS_DIR / "one_hot_reference_categories.csv",
        index=False,
    )
    ...
    for col in encoded_cols:
        if clean_df[col].nunique(dropna=True) > 1:
            corr = clean_df[[col, TARGET_COL]].corr().iloc[0, 1]
        else:
            corr = np.nan
        corr_rows.append({"feature": col, "pearson_corr_with_target": corr})

    pd.DataFrame(corr_rows).sort_values(
        "pearson_corr_with_target",
        key=lambda s: s.abs(),
        ascending=False,
    ).to_csv(ANALYSIS_DIR / "correlation_with_target_final_features.csv", index=False)

    clean_dataset_path = DATASETS_DIR / "real1_cuant_categ_clean.csv"
    clean_df.to_csv(clean_dataset_path, index=False)

    report_lines = []
    report_lines.append("ANÁLISIS INICIAL DEL DATASET REAL 1 - CUANTITATIVAS + CATEGÓRICAS")
    report_lines.append("=" * 80)
    report_lines.append(f"Archivo leído: {DATA_FILE_PATH}")
    report_lines.append(f"Filas originales del Excel: {cleaning_info.get('original_rows_excel', len(raw_df))}")
    report_lines.append(f"Columnas originales del Excel: {cleaning_info.get('original_columns_excel', raw_df.shape[1])}")
    report_lines.append(f"Columnas tipo Unnamed eliminadas: {cleaning_info.get('unnamed_columns_removed', 0)}")
    report_lines.append(f"Columnas completamente vacías eliminadas: {cleaning_info.get('empty_columns_removed', 0)}")
    report_lines.append(f"Filas completamente vacías eliminadas: {cleaning_info.get('empty_rows_removed', 0)}")
    report_lines.append(f"Duplicados exactos eliminados: {n_duplicates_removed}")
    report_lines.append(f"Filas eliminadas por target ausente: {n_target_na_removed}")
    report_lines.append(f"Filas eliminadas por missing en variables seleccionadas: {cleaning_info.get('rows_removed_missing_selected_features', 0)}")
    report_lines.append(f"Filas finales disponibles para el modelo: {len(clean_df)}")
    report_lines.append(f"Columnas tras limpieza básica: {raw_df.shape[1]}")
    report_lines.append(f"Variable objetivo: {TARGET_COL}")
    report_lines.append("")
    report_lines.append("Distribución de clases:")
    for _, row in class_distribution_df.iterrows():
        report_lines.append(
            f"  Clase {int(row['class'])}: {int(row['count'])} muestras "
            f"({row['proportion'] * 100:.2f}%)"
    ...
    report_lines.append("")
    report_lines.append(f"Número de variables cuantitativas originales seleccionadas: {len(quantitative_cols)}")
    report_lines.append("Variables cuantitativas originales usadas:")
    for col in quantitative_cols:
        report_lines.append(
            f"  - {col} | missing antes dropna={int(raw_df[col].isna().sum())} "
            f"({float(raw_df[col].isna().mean() * 100):.2f}%) | "
            f"n_unique={int(raw_df[col].nunique(dropna=True))}"
        )

    report_lines.append("")
    report_lines.append(f"Número de variables categóricas originales seleccionadas: {len(categorical_cols)}")
    report_lines.append(f"Categóricas binarias transformadas directamente a 0/1: {len(binary_categorical_cols)}")
    report_lines.append(f"Categóricas multiclase codificadas mediante One-Hot con referencia: {len(multiclass_categorical_cols)}")
    report_lines.append(f"Criterio de cardinalidad categórica: n_unique <= {MAX_CATEGORICAL_UNIQUE_VALUES}")

    report_lines.append("")
    report_lines.append("Variables categóricas binarias transformadas a 0/1:")
    if binary_mapping_rows:
        for item in binary_mapping_rows:
            report_lines.append(
                f"  - {item['feature']}: {item['category_mapped_to_0']} -> 0, "
                f"{item['category_mapped_to_1']} -> 1"
            )
    else:
        report_lines.append("  - No se detectó ninguna categórica binaria.")

    report_lines.append("")
    report_lines.append("Variables categóricas multiclase codificadas mediante One-Hot con categoría de referencia:")
    if one_hot_reference_rows:
        for item in one_hot_reference_rows:
            report_lines.append(
                f"  - {item['feature']} | referencia omitida={item['reference_category']} "
                f"| categorías codificadas={item['encoded_categories']}"
            )
    else:
        report_lines.append("  - No se detectó ninguna categórica multiclase seleccionada.")

    report_lines.append("")
    report_lines.append(f"Número final de variables predictoras tras codificación: {len(encoded_cols)}")
    report_lines.append(f"Columnas constantes eliminadas tras codificación: {len(constant_encoded_cols)}")
    report_lines.append("")
    report_lines.append(f"Filas con algún missing en variables seleccionadas antes de eliminar filas: {cleaning_info.get('rows_with_missing_in_selected_features_before_dropna', 0)}")
    report_lines.append(f"Valores perdidos totales en variables seleccionadas antes de eliminar filas: {cleaning_info.get('missing_values_in_selected_features_before_dropna', 0)}")
    report_lines.append(f"Filas eliminadas por missing en variables seleccionadas: {cleaning_info.get('rows_removed_missing_selected_features', 0)}")
    report_lines.append("")
    report_lines.append("Nota: siguiendo el criterio indicado, no se aplica imputación; las filas con valores perdidos en variables seleccionadas se eliminan antes del entrenamiento.")
    report_lines.append("Nota: las categóricas binarias se transforman directamente a 0/1; solo las categóricas con más de dos categorías se codifican mediante One-Hot dejando una categoría fija como referencia.")

    with open(ANALYSIS_DIR / "dataset_initial_analysis_summary.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))

    save_dataset_analysis_figures(clean_df, encoded_cols, class_distribution_df)


def load_and_analyze_real_dataset(force_recompute=False):
    """
    Carga, analiza y prepara el dataset real 1 usando variables cuantitativas y categóricas.

    Limpieza aplicada:
    - Lee el Excel indicado.
    ...
    - Comprueba que la variable objetivo sea binaria 0/1.
    - Selecciona variables cuantitativas como predictores.
    - Selecciona variables categóricas no numéricas de cardinalidad moderada como predictores.
    - Excluye la variable objetivo, columnas constantes y categóricas de cardinalidad excesiva.
    - Elimina directamente las filas con valores perdidos en las variables seleccionadas.
      No se aplica imputación.
    - Transforma las variables categóricas binarias directamente a 0/1.
    - Codifica las variables categóricas multiclase mediante One-Hot Encoding con categoría de referencia fija.
    """
    global _CLEAN_DF_CACHE, _FEATURE_COLS_CACHE, _RAW_DF_CACHE, _EXCLUDED_COLS_CACHE, _CLEANING_INFO_CACHE

    if (
        not force_recompute
        and _CLEAN_DF_CACHE is not None
        and _FEATURE_COLS_CACHE is not None
        and _RAW_DF_CACHE is not None
    ):
        return _CLEAN_DF_CACHE.copy(), list(_FEATURE_COLS_CACHE), _RAW_DF_CACHE.copy(), list(_EXCLUDED_COLS_CACHE)

    if not DATA_FILE_PATH.exists():
        raise FileNotFoundError(
            f"No se encuentra el archivo {DATA_FILE_NAME}. "
            f"Colócalo en la misma carpeta que este script: {PROJECT_DIR}"
        )

    raw_excel_df = pd.read_excel(DATA_FILE_PATH, sheet_name=EXCEL_SHEET_NAME)
    n_rows_original = int(raw_excel_df.shape[0])
    n_cols_original = int(raw_excel_df.shape[1])

    raw_df = _clean_column_names(raw_excel_df)
    raw_df = raw_df.replace(r"^\s*$", np.nan, regex=True)

    unnamed_cols = [col for col in raw_df.columns if str(col).startswith("Unnamed")]
    n_unnamed_cols_removed = len(unnamed_cols)
    raw_df = raw_df.drop(columns=unnamed_cols, errors="ignore")

    n_cols_before_empty_drop = int(raw_df.shape[1])
    raw_df = raw_df.dropna(axis=1, how="all")
    n_empty_cols_removed = n_cols_before_empty_drop - int(raw_df.shape[1])

    n_rows_before_empty_drop = int(raw_df.shape[0])
    raw_df = raw_df.dropna(axis=0, how="all").reset_index(drop=True)
    n_empty_rows_removed = n_rows_before_empty_drop - int(raw_df.shape[0])

    if TARGET_COL not in raw_df.columns:
        raise ValueError(f"No existe la columna objetivo {TARGET_COL} en el dataset.")

    n_rows_before_duplicates = int(len(raw_df))
    raw_df = raw_df.drop_duplicates().reset_index(drop=True)
    n_duplicates_removed = n_rows_before_duplicates - int(len(raw_df))

    n_rows_before_target_na = int(len(raw_df))
    raw_df = raw_df.dropna(subset=[TARGET_COL]).reset_index(drop=True)
    n_target_na_removed = n_rows_before_target_na - int(len(raw_df))

    raw_df[TARGET_COL] = _coerce_binary_target(raw_df[TARGET_COL])
    raw_df = raw_df.replace([np.inf, -np.inf], np.nan)

    numeric_cols = raw_df.select_dtypes(include=[np.number]).columns.tolist()
    quantitative_feature_cols_initial = [col for col in numeric_cols if col != TARGET_COL]

    constant_numeric_cols = [
        col for col in quantitative_feature_cols_initial
        if raw_df[col].nunique(dropna=True) <= 1
    ]

    quantitative_feature_cols = [
        col for col in quantitative_feature_cols_initial
        if col not in constant_numeric_cols
    ]
    ...
    categorical_candidate_cols = [
        ...
        if (
            ...
            and not pd.api.types.is_datetime64_any_dtype(raw_df[col])
        )
    ]
    ...
    high_cardinality_categorical_cols = [
        col for col in categorical_candidate_cols
        if (
            col not in constant_categorical_cols
            and raw_df[col].nunique(dropna=True) > MAX_CATEGORICAL_UNIQUE_VALUES
        )
    ]
    ...
    categorical_feature_cols = [
        ...
        if (
            col not in constant_categorical_cols
            and col not in high_cardinality_categorical_cols
            and raw_df[col].nunique(dropna=True) >= 2
        )
    ]

    selected_original_feature_cols = quantitative_feature_cols + categorical_feature_cols
    ...
    for col in raw_df.columns:
        if col == TARGET_COL:
        ...
        elif col in quantitative_feature_cols:
            continue
        elif col in categorical_feature_cols:
            continue
        elif col in constant_numeric_cols or col in constant_categorical_cols:
            excluded_cols.append({
                "column": col,
        ...
        elif col in high_cardinality_categorical_cols:
            excluded_cols.append({
                "column": col,
                "reason": f"categorical_high_cardinality_gt_{MAX_CATEGORICAL_UNIQUE_VALUES}",
                "dtype": str(raw_df[col].dtype),
                "n_unique": int(raw_df[col].nunique(dropna=True)),
                "missing_count": int(raw_df[col].isna().sum()),
                "missing_pct": float(raw_df[col].isna().mean()),
            ...
            excluded_cols.append({
                "column": col,
                "reason": "excluded_not_selected_as_predictor",
                "dtype": str(raw_df[col].dtype),
                "n_unique": int(raw_df[col].nunique(dropna=True)),
                "missing_count": int(raw_df[col].isna().sum()),
                "missing_pct": float(raw_df[col].isna().mean()),
    ...
    if not selected_original_feature_cols:
        raise ValueError("No se han encontrado variables predictoras para entrenar el modelo.")

    clean_df_before_dropna = raw_df[selected_original_feature_cols + [TARGET_COL]].copy()

    n_rows_with_missing_selected_features = int(
        clean_df_before_dropna[selected_original_feature_cols].isna().any(axis=1).sum()
    )
    n_missing_selected_features_total = int(
        clean_df_before_dropna[selected_original_feature_cols].isna().sum().sum()
    )

    # Criterio indicado para el dataset real: si hay missing en variables
    # seleccionadas, se elimina directamente la fila. No se aplica imputación.
    n_rows_before_feature_dropna = int(len(clean_df_before_dropna))
    clean_original_df = clean_df_before_dropna.dropna(
        subset=selected_original_feature_cols
    ).reset_index(drop=True)
    n_rows_removed_missing_selected_features = n_rows_before_feature_dropna - int(len(clean_original_df))

    if clean_original_df[TARGET_COL].nunique(dropna=True) < 2:
        raise ValueError(
            "Tras eliminar filas con valores perdidos en variables seleccionadas, "
            "la variable objetivo queda con menos de dos clases."
        )
    ...
    binary_mapping_rows = []
    one_hot_reference_rows = []
    one_hot_category_order = {}

    for col in categorical_feature_cols:
        categories = sorted(
            clean_original_df[col].dropna().astype(str).str.strip().unique().tolist()
        )

        if len(categories) == 2:
            binary_categorical_cols.append(col)
            mapping = {categories[0]: 0, categories[1]: 1}
            clean_original_df[col] = clean_original_df[col].map(mapping).astype(int)

            binary_mapping_rows.append({
                "feature": col,
                "category_mapped_to_0": categories[0],
                "category_mapped_to_1": categories[1],
                "n_categories": 2,
            })

        elif len(categories) > 2:
            multiclass_categorical_cols.append(col)
            reference_category = REFERENCE_CATEGORIES.get(col, categories[0])

            if reference_category not in categories:
                raise ValueError(
                    f"La categoría de referencia '{reference_category}' definida para {col} "
                    f"no existe en el dataset. Categorías encontradas: {categories}"
                )

            encoded_categories = [cat for cat in categories if cat != reference_category]
            one_hot_category_order[col] = [reference_category] + encoded_categories

            one_hot_reference_rows.append({
                "feature": col,
                "reference_category": reference_category,
                "encoded_categories": "; ".join(encoded_categories),
                "n_categories": len(categories),
            })

    if multiclass_categorical_cols:
        for col in multiclass_categorical_cols:
            clean_original_df[col] = pd.Categorical(
                clean_original_df[col],
                categories=one_hot_category_order[col],
                ordered=False,
            )
    ...
    encoded_feature_cols = [col for col in encoded_df.columns if col != TARGET_COL]

    constant_encoded_cols = [
        col for col in encoded_feature_cols
        if encoded_df[col].nunique(dropna=True) <= 1
    ]

    if constant_encoded_cols:
        encoded_df = encoded_df.drop(columns=constant_encoded_cols)
        encoded_feature_cols = [col for col in encoded_feature_cols if col not in constant_encoded_cols]

    clean_df = encoded_df[encoded_feature_cols + [TARGET_COL]].copy()

    cleaning_info = {
        "original_rows_excel": n_rows_original,
        "original_columns_excel": n_cols_original,
        "columns_after_name_cleaning": int(raw_excel_df.shape[1]),
        "unnamed_columns_removed": n_unnamed_cols_removed,
        "empty_columns_removed": int(n_empty_cols_removed),
        "empty_rows_removed": int(n_empty_rows_removed),
        "duplicates_removed": int(n_duplicates_removed),
        "rows_removed_missing_target": int(n_target_na_removed),
        "rows_after_basic_cleaning": int(len(raw_df)),
        "columns_after_basic_cleaning": int(raw_df.shape[1]),
        "rows_before_dropna_selected_features": int(n_rows_before_feature_dropna),
        "rows_removed_missing_selected_features": int(n_rows_removed_missing_selected_features),
        "rows_final_model_dataset": int(len(clean_df)),
        "selected_quantitative_features": int(len(quantitative_feature_cols)),
        "selected_categorical_features": int(len(categorical_feature_cols)),
        "selected_binary_categorical_features": int(len(binary_categorical_cols)),
        "selected_multiclass_categorical_features": int(len(multiclass_categorical_cols)),
        "encoded_predictor_features": int(len(encoded_feature_cols)),
        "constant_encoded_columns_removed": int(len(constant_encoded_cols)),
        "excluded_columns_total_including_target": int(len(excluded_cols)),
        "rows_with_missing_in_selected_features_before_dropna": n_rows_with_missing_selected_features,
        "missing_values_in_selected_features_before_dropna": n_missing_selected_features_total,
        "max_categorical_unique_values": int(MAX_CATEGORICAL_UNIQUE_VALUES),
        "quantitative_feature_cols": list(quantitative_feature_cols),
        "categorical_feature_cols": list(categorical_feature_cols),
        "binary_categorical_cols": list(binary_categorical_cols),
        "multiclass_categorical_cols": list(multiclass_categorical_cols),
        "binary_mapping_rows": list(binary_mapping_rows),
        "one_hot_reference_rows": list(one_hot_reference_rows),
        "one_hot_category_order": dict(one_hot_category_order),
        "encoded_feature_cols": list(encoded_feature_cols),
        "constant_encoded_cols": list(constant_encoded_cols),
    }

    save_dataset_analysis_outputs(
        raw_df=raw_df,
        clean_df=clean_df,
        feature_cols=encoded_feature_cols,
        excluded_cols=excluded_cols,
        n_duplicates_removed=n_duplicates_removed,
        n_target_na_removed=n_target_na_removed,
        cleaning_info=cleaning_info,
    )

    _CLEAN_DF_CACHE = clean_df.copy()
    _FEATURE_COLS_CACHE = list(encoded_feature_cols)
    _RAW_DF_CACHE = raw_df.copy()
    _EXCLUDED_COLS_CACHE = list(excluded_cols)
    _CLEANING_INFO_CACHE = dict(cleaning_info)

    return clean_df.copy(), list(encoded_feature_cols), raw_df.copy(), list(excluded_cols)


def get_real_dataset_and_features():
    clean_df, feature_cols, _, _ = load_and_analyze_real_dataset(force_recompute=False)
    return clean_df, feature_cols


def print_initial_dataset_analysis_once():
    """
    Ejecuta el análisis exploratorio una sola vez en el proceso principal y lo muestra por terminal.

    Se fuerza una recomputación inicial para que las tablas/figuras exploratorias
    estén actualizadas antes de empezar TRAIN. Después, el dataset queda en caché.
    """
    clean_df, feature_cols, raw_df, excluded_cols = load_and_analyze_real_dataset(force_recompute=True)
    ...
    print_dataset_analysis_terminal_summary(
        raw_df=raw_df,
        clean_df=clean_df,
        feature_cols=feature_cols,
        class_distribution_df=class_distribution_df,
        excluded_cols=excluded_cols,
        cleaning_info=_CLEANING_INFO_CACHE,
    )
...
def load_or_create_dataset(seed, scenario_name, config):
    """
    Para el dataset real no se generan muestras nuevas.
    Cada semilla crea una partición estratificada distinta del mismo dataset real.
    ...
    - 10% test
    """
    clean_df, feature_cols = get_real_dataset_and_features()

    scenario_dataset_dir = DATASETS_DIR / get_short_scenario_name(scenario_name)
    ...
    df_train["sample_id"] = idx_train

    df_val = clean_df.iloc[idx_val].copy()
    df_val["split"] = "val"
    df_val["sample_id"] = idx_val

    df_test = clean_df.iloc[idx_test].copy()
    df_test["split"] = "test"
    df_test["sample_id"] = idx_test
    ...
    for split_name in ["train", "val", "test"]:
        ...
        counts = temp[TARGET_COL].value_counts().sort_index()
        for class_value in [0, 1]:
            split_distribution_rows.append({
                ...
                "proportion": float((temp[TARGET_COL] == class_value).mean()),
            })

    split_dist_path = scenario_dataset_dir / f"real1_cuant_categ_seed{seed:02d}_split.csv"
    pd.DataFrame(split_distribution_rows).to_csv(split_dist_path, index=False)
...
def compute_balanced_points_by_scenario():
    ...
    Como hay varias particiones/seeds definidas en SEEDS, guardamos media y desviación típica de los pesos. Este punto se marca después en las figuras de validación mean.
    """
    rows = []
...
def run_pandas_numpy_metrics_warmup():
    ...
    try:
        ...
        warmup_df = pd.DataFrame({
            "scenario": np.repeat(REAL_SCENARIO_NAME, n_rows),
...
def preload_datasets_for_timing():
    ...
    lectura del Excel, creación de particiones o caché del sistema operativo.
    No se incluye en el coste comparable porque es calentamiento técnico común.
    """
    t_start = time.perf_counter()
    n_loaded = 0
    ...
    try:
        ...
        elapsed = time.perf_counter() - t_start
        print(f"Precarga técnica de datasets terminada: {n_loaded} particiones/seeds · {format_seconds(elapsed)}. No se incluye en el coste comparable.")
    except Exception as exc:
...
def get_coefficients_wide_path(scenario_name, strategy_name):
    ...
    if strategy_name == "sklearn_balanced":
        return SKLEARN_BALANCED_COEFFICIENTS_WIDE_DIR / f"{short_name}_sklearn_balanced_coeffs.csv"
...
def train_or_load_final_strategy_coefficients(timer_frames):
    ...
    print(f"El script entrena una estrategia, termina ese bloque y después pasa a la siguiente. Dentro de cada bloque se paralelizan las {len(SEEDS)} particiones/seeds.\n")
    ...
    for scenario_name, config in SCENARIOS.items():
        for strategy_name in ["equal_weights", "sklearn_balanced"]:
            ...
            print("=" * 80)
            print(f"Ejecutando {len(SEEDS)} particiones/seeds en paralelo...")
            print("Modelos entrenados: 1 por partición/seed")
...
def draw_contrast_reference_line(ax, x0, y0, x1, y1, color):
    ...
    La línea queda por debajo de las estrellas y de los recuadros. Así, aunque
    se modifique C_VALUES y cambie la posición de los puntos, la unión visual
    no pisa el marcador: la estrella se dibuja por encima y tapa el extremo de
    la línea si llegan a tocarse.
    """
    ...
    # Deja una separación visual mayor junto a la estrella. Además, como la
    # línea se dibuja con zorder inferior al marcador, nunca queda por encima
    # de la estrella aunque el recorte sea muy corto.
    marker_gap = min(0.52, 0.28 * length)
    box_gap = min(0.14, 0.10 * length)
...
def draw_single_star_marker_2d(ax, x, y, color, size, angle_degrees=0.0, zorder=24):
    """
...
def add_pins_to_heatmap(ax, pivot, best_point, balanced_point, metric, scenario_name, val_comparison_df=None, equal_point=None):
    ...
    Orden visual usado:
    1) recuadros de texto;
    2) líneas simples de referencia, con zorder bajo;
    3) estrellas al final, por encima de las líneas.

    Así las líneas nunca quedan encima de las estrellas, pero siguen conectando
    el borde real del recuadro con el punto correspondiente.
    """
    c0_values = pivot.index.to_numpy(dtype=float)
    c1_values = pivot.columns.to_numpy(dtype=float)
    ...
    # Primero se dibujan los recuadros para poder medir su bbox real.
    for item in points:
        point = item["point"]
    ...
    # Después se dibujan las líneas con zorder bajo. Quedan por debajo de las
    # estrellas, por lo que no ensucian el marcador aunque pasen cerca.
    ...
    # Finalmente se dibujan las estrellas por encima de las líneas. Si coinciden,
    # se mantienen en la misma coordenada y se diferencian por rotación visual.
...
def add_3d_pin_legend(fig, metric, pin_info):
    ...
    superficie es una representación visual entre esos puntos.
    """
    handles = []
...
def save_final_comparison_plots(final_agg_df, split_name):
    ...
    for metric in metrics_to_plot:
        ...
        ax.set_title(
            f"Comparación final en {SPLIT_DISPLAY_NAMES.get(split_name, str(split_name).upper())} · {format_metric_label_for_plot(metric, include_split=False)}",
            fontsize=14,
            pad=12,
        )
...
def _draw_confusion_matrix_on_axis(ax, cm, title):
    """
    Dibuja una matriz de confusión 2x2 normalizada por fila real.

    En cada celda se muestra:
    - conteo absoluto;
    - porcentaje dentro de la clase real correspondiente.
    """
...
def save_final_confusion_matrix_panels(predictions_path, split_name):
    """
    Guarda matrices de confusión finales en figures/comparativas_globales.

    En Real 1 hay un único escenario. Por eso se genera un panel por split
    comparando las tres estrategias finales:
    - Manual Best F1
    - Equal weights
    - Sklearn balanced

    No modifica entrenamiento, evaluación ni métricas. Solo representa las
    predicciones finales ya calculadas.
    """
    ...
    fig, axes = plt.subplots(1, len(available_models), figsize=(5.4 * len(available_models), 5.4))
    ...
    for ax, model_name in zip(axes, available_models):
        style = get_model_plot_style(model_name)
        group = predictions_df[predictions_df["model_name"] == model_name].copy()
        ...
        if "seed" in group.columns:
            ...
            title = (
                f"{style['label']}\n"
                f"{str(split_name).upper()} · agregada {n_seeds_group} particiones/seeds\n"
                f"n total={n_total}, positivos={n_pos}"
            )
        else:
            title = (
                f"{style['label']}\n"
                f"{str(split_name).upper()} · agregada\n"
                f"n total={n_total}, positivos={n_pos}"
            )
    ...
    if "seed" in predictions_df.columns:
        n_seeds_panel = int(predictions_df["seed"].nunique())
        aggregation_text = f"agregadas entre {n_seeds_panel} particiones/seeds"
        footer_text = (
            f"Lectura: cada matriz acumula las predicciones finales de {str(split_name).upper()} de {n_seeds_panel} particiones/seeds; "
            "el n mostrado es el total agregado y cada celda muestra conteo acumulado y porcentaje por fila real."
        )
    else:
    ...
    fig.suptitle(
        f"{SPLIT_DISPLAY_NAMES.get(str(split_name), str(split_name).upper())} FINAL · Matrices de confusión {aggregation_text} · {format_scenario_label_for_plot(REAL_SCENARIO_NAME)}",
        fontsize=18,
        y=0.985,
    )
    ...
    if last_im is not None:
        cbar_ax = fig.add_axes([0.925, 0.230, 0.018, 0.450])
        cbar = fig.colorbar(last_im, cax=cbar_ax)
        cbar.set_label("Porcentaje dentro de cada clase real", rotation=90, labelpad=12)

    fig.savefig(
        get_global_figure_path(f"{split_name}_confusion_matrix_panel_all_models.png"),
        dpi=180,
        bbox_inches="tight",
        pad_inches=0.12,
    )
...
def save_final_boxplot_plots_and_tables(final_results_df, split_name):
    ...
    métrica a lo largo de las particiones/seeds definidas en SEEDS.
    ...
    for metric in metrics_to_plot:
        ...
        ax.set_ylabel(format_metric_axis_label_for_plot(metric))
        ax.set_title(
            f"Caja y bigotes en {SPLIT_DISPLAY_NAMES.get(split_name, str(split_name).upper())} · {format_metric_label_for_plot(metric, include_split=False)}",
            fontsize=14,
            pad=12,
        )
...
def remove_old_incompatible_outputs():
    ...
    for scenario_name in SCENARIOS.keys():
        ...
        old_coeff_path = COEFFICIENTS_WIDE_DIR / f"{get_short_scenario_name(scenario_name)}_coef.csv"
...
def save_timers(timer_frames):
    ...
    Se eliminan del archivo de tiempos los tiempos internos acumulados por
    modelo/seed porque no representan el tiempo real observado en pantalla y
    pueden confundir el análisis de ahorro computacional.
...
def print_clean_timer_summary(timers_df):
    """
    Resumen de tiempos reales comparables del dataset real 1 con variables
    ...
    print("\n" + "=" * 128)
    print("RESUMEN DE COSTE COMPUTACIONAL COMPARABLE · DATASET REAL 1 · CUANTITATIVAS + CATEGÓRICAS")
    print("=" * 128)
    ...
    print("- Agregación/selección: solo aplica a Weighted grid; calcula mean/std entre particiones/seeds y elige el mejor par por val_f1_mean.")
    ...
    print(f"- En este dataset real no se generan muestras nuevas: las {n_seeds} seeds crean {n_seeds} particiones train/val/test distintas del mismo Excel.")
    print(f"- Dentro de cada bloque se paralelizan las {n_seeds} particiones/seeds con joblib; no se paralelizan todos los bloques a la vez.")
    print(f"- Rejilla weighted actual: {len(C_VALUES)} x {len(C_VALUES)} = {grid_size} combinaciones de pesos por partición/seed x {n_seeds} seeds.")
    ...
    print("   Weighted grid no remuestrea; su búsqueda consiste en entrenar la rejilla, validarla, agregar las particiones/seeds y seleccionar Best F1.")
    ...
    print("   Equal weights y Sklearn balanced entrenan un único modelo por partición/seed.")
    print(f"   Rejilla completa: {len(C_VALUES)} x {len(C_VALUES)} = {grid_size} pesos por partición/seed x {n_seeds} seeds.")
    ...
    print("\n0.3) AGREGACIÓN Y SELECCIÓN BEST F1")
    print(f"   Agrega mean/std entre las {n_seeds} particiones/seeds y selecciona el mayor val_f1_mean.")
...
def main():
    ...
    # Primero se muestra un único análisis exploratorio breve.
    # Después comienza el flujo habitual de TRAIN, VALIDACIÓN, TEST y figuras.
    print_initial_dataset_analysis_once()
    ...
    if existing_val_grid_outputs_are_compatible():
        ...
        # Aseguramos que los datasets existen para figuras y compatibilidad de salidas.
        for scenario_name, config in SCENARIOS.items():
            for seed in SEEDS:
    ...
    # La selección se desglosa por escenario para que la terminal muestre tiempos reales separados.
    best_rows = []
        ...
        for scenario_name, config in SCENARIOS.items():
            ...
            for model_name in ["best_val_f1_grid", "equal_weights", "sklearn_balanced"]:
                ...
                if model_name == "best_val_f1_grid":
                    ...
                    print("Esta estrategia SÍ usa esos pesos manuales seleccionados en validación.")
                    ...
                    print(
                        ...
                        f"(std: c0={balanced_row['balanced_c0_std']:.6g}, "
        ...
        for scenario_name, config in SCENARIOS.items():
            ...
            for model_name in ["best_val_f1_grid", "equal_weights", "sklearn_balanced"]:
                ...
                if model_name == "best_val_f1_grid":
                    ...
                    print("Esta estrategia SÍ usa esos pesos manuales para la evaluación final en TEST.")
