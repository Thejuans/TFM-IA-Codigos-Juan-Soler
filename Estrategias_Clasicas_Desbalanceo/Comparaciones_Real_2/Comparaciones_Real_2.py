...
# ============================================================
# 1. CONFIGURACIÓN REAL 2 · PARTICIÓN CLÁSICA · RISK 1 Y RISK 2
# ============================================================
...
def get_target_col(scenario_name=None):
    if scenario_name is None:
        return TARGET_COLS[0]
    return SCENARIOS[str(scenario_name)]["target"]
...
def validate_required_base_flow():
    """
    Comprueba que existen los datasets y CSV necesarios del flujo WBCE Real 2.

    Este código de comparación NO necesita leer de nuevo el Excel original si
    ya se ha ejecutado antes el código base WBCE de Real 2. Las estrategias
    nuevas se entrenan usando exactamente los CSV de particiones por seed que
    dejó guardados el código base.
    """
    missing = []
    ...
    for path in [
        ...
        BASE_OUTPUT_DIR / "timers_execution.txt",
    ]:
        if not path.exists():
            missing.append(path)
    ...
    for scenario_name in SCENARIOS.keys():
        for seed in SEEDS:
            dataset_path = get_seed_dataset_path(scenario_name, seed)
    ...
    if missing:
        stop_missing_base_flow(
            ...
            required_files=[
                ...
                "test_final_predictions_raw_multiseed.csv",
                "timers_execution.txt",
                "datasets/r2_risk1/r2_risk1_seedXX.csv",
                "datasets/r2_risk2/r2_risk2_seedXX.csv",
            ],
            searched_roots=[PROJECT_DIR] + list(PROJECT_DIR.parents),
            extra_message=(
...
def get_feature_cols_from_df(df):
    excluded = {"split", "sample_id", ID_COL} | set(TARGET_COLS)
    excluded.update({c for c in df.columns if str(c).strip().startswith("Suicide_Risk")})
    return [c for c in df.columns if c not in excluded]


def load_or_create_dataset(seed, scenario_name, config):
    """
    Carga la partición ya generada por el código base WBCE Real 2.

    No se vuelve a limpiar el Excel ni se regeneran splits desde cero: así se
    garantiza que RandomOverSampler, SMOTE y RandomUnderSampler se comparen con
    las mismas 20 particiones usadas por WBCE, Equal weights y Sklearn balanced.
    """
    dataset_path = get_seed_dataset_path(scenario_name, seed)
    ...
    if not dataset_path.exists():
        raise FileNotFoundError(
            f"No se encuentra el dataset de partición base: {dataset_path}\n"
            "Ejecuta primero el código base WBCE Real 2 para generar las particiones por seed."
        )

    df = pd.read_csv(dataset_path)
    required_cols = {target_col, "split", "sample_id"}
    missing_cols = required_cols - set(df.columns)
    if missing_cols:
        raise ValueError(
            f"El dataset base {dataset_path} no contiene las columnas requeridas: {sorted(missing_cols)}"
        )

    feature_cols = get_feature_cols_from_df(df)
    if not feature_cols:
        raise ValueError(f"No se han encontrado variables predictoras en {dataset_path}")
...
def get_total_single_strategy_train_models():
    """
    Número total de modelos entrenados en TRAIN por estrategias con un único
    ajuste de LogisticRegression por seed.
    """
    return int(len(SEEDS))


def compute_greedy_cv_models_from_pairs(greedy_pairs, greedy_folds=5):
    """
    Número de ajustes internos realizados por la búsqueda greedy:
    pares de pesos evaluados x folds de validación cruzada.
    """
    if pd.isna(greedy_pairs):
        return np.nan
    return int(greedy_pairs) * int(greedy_folds)


def compute_greedy_total_models_from_pairs(greedy_pairs, greedy_folds=5):
    """
    Número total de modelos de la heurística greedy por seed:
    modelos de CV interna + refit final sobre TRAIN.
    """
    greedy_cv_models = compute_greedy_cv_models_from_pairs(greedy_pairs, greedy_folds)
    if pd.isna(greedy_cv_models):
        return np.nan
    return int(greedy_cv_models) + 1


def get_greedy_total_models_from_coefficients(scenario_name):
    """
    Recupera el número exacto de modelos de la heurística greedy desde el CSV
    de coeficientes, útil cuando los coeficientes ya existen y se reutilizan.
    """
    try:
        ...
        df = pd.read_csv(path)
        df = df[
            (df["scenario"].astype(str) == str(scenario_name))
            & (df["model_name"].astype(str) == "greedy_class_weight")
        ].copy()
        if df.empty or "greedy_weight_pairs_evaluated" not in df.columns:
            return 0

        if "greedy_cv_folds" in df.columns:
            folds = pd.to_numeric(df["greedy_cv_folds"], errors="coerce").fillna(5).astype(int)
        else:
            folds = pd.Series([5] * len(df), index=df.index)

        pairs = pd.to_numeric(df["greedy_weight_pairs_evaluated"], errors="coerce")
        valid = pairs.notna()
        if valid.sum() == 0:
            return 0

        total = (pairs[valid].astype(int) * folds[valid].astype(int) + 1).sum()
        return int(total)
    except Exception:
        return 0


def _count_values_in_text(value):
    """
    Cuenta valores numéricos en una representación textual de una lista.
    Se usa solo para recuperar automáticamente el tamaño de C_VALUES si existe
    en archivos de configuración del flujo base.
    """
    ...
    return len(numbers) if numbers else np.nan


def _search_grid_size_in_json(obj):
    """
    Busca de forma recursiva el tamaño de la rejilla en un metadata JSON.
    """
    if isinstance(obj, dict):
        ...
        for key in [
            ...
            "n_weight_pairs",
            "n_models_per_seed",
            "grid_models_per_seed",
            "n_grid_models_per_seed",
        ]:
            if key in lower_keys:
                try:
                    return int(obj[lower_keys[key]])
                except Exception:
                    pass

        for key in ["n_c_values", "n_cvalues", "num_c_values"]:
            if key in lower_keys:
                try:
                    n_values = int(obj[lower_keys[key]])
                    return int(n_values * n_values)
                except Exception:
                    pass

        for key in ["c_values", "cvalues", "c_values_grid"]:
            if key in lower_keys:
                value = obj[lower_keys[key]]
                if isinstance(value, (list, tuple)):
                    return int(len(value) * len(value))
                n_values = _count_values_in_text(value)
                if not pd.isna(n_values):
                    return int(n_values * n_values)

        for value in obj.values():
            result = _search_grid_size_in_json(value)
...
def get_base_grid_models_per_seed_from_outputs():
    """
    Intenta recuperar automáticamente el número de combinaciones de la rejilla
    del flujo base WBCE. Si no existe un archivo de configuración compatible,
    devuelve NaN para evitar inventar un valor.
    """
    candidate_csvs = [
        BASE_OUTPUT_DIR / "scenarios_config.csv",
        BASE_OUTPUT_DIR / "scenario_config.csv",
        BASE_OUTPUT_DIR / "config.csv",
    ]
    ...
    for path in candidate_csvs:
        ...
        for col_name in ["n_c_values", "n_cvalues", "num_c_values"]:
            if col_name in lower_cols:
                values = pd.to_numeric(df[lower_cols[col_name]], errors="coerce").dropna()
                if not values.empty:
                    n_values = int(values.iloc[0])
                    return int(n_values * n_values)

        for col_name in ["c_values", "cvalues", "c_values_grid"]:
            if col_name in lower_cols:
                n_values = _count_values_in_text(df[lower_cols[col_name]].dropna().iloc[0])
                if not pd.isna(n_values):
                    return int(n_values * n_values)

        c0_cols = [c for c in df.columns if str(c).lower() in {"c0_values", "c0_grid"}]
        c1_cols = [c for c in df.columns if str(c).lower() in {"c1_values", "c1_grid"}]
        if c0_cols and c1_cols:
            n_c0 = _count_values_in_text(df[c0_cols[0]].dropna().iloc[0])
            n_c1 = _count_values_in_text(df[c1_cols[0]].dropna().iloc[0])
            if not pd.isna(n_c0) and not pd.isna(n_c1):
                return int(n_c0 * n_c1)

    candidate_jsons = [
        BASE_OUTPUT_DIR / "metadata.json",
        BASE_OUTPUT_DIR / "experiment_metadata.json",
        BASE_OUTPUT_DIR / "run_metadata.json",
    ]
    for path in candidate_jsons:
        ...
        try:
            ...
            continue
        result = _search_grid_size_in_json(data)
        if not pd.isna(result):
            return int(result)
...
def get_expected_base_models_for_strategy(model_name):
    """
    Número esperado de modelos entrenados en TRAIN para estrategias del flujo base.
    """
    if model_name == "best_val_f1_grid":
        grid_models_per_seed = get_base_grid_models_per_seed_from_outputs()
        if pd.isna(grid_models_per_seed):
            return np.nan
        return int(len(SEEDS) * int(grid_models_per_seed))

    if model_name in ["equal_weights", "sklearn_balanced"]:
        return get_total_single_strategy_train_models()
    ...
    try:
        for scenario_name, config in get_scenarios().items():
            for seed in SEEDS:
                ...
                feature_cols = get_feature_cols_from_df(df)
                target_col = get_target_col(scenario_name)
...
def train_one_seed_greedy_strategy(seed, scenario_name, config):
    ...
    greedy_folds = 5
    greedy_cv_models = compute_greedy_cv_models_from_pairs(greedy_pairs, greedy_folds)
    greedy_total_models = compute_greedy_total_models_from_pairs(greedy_pairs, greedy_folds)
...
def train_or_load_greedy_strategy(scenario_name, config, timer_frames):
    ...
    if coefficients_are_compatible_for_strategy(scenario_name, "greedy_class_weight", feature_cols):
        ...
        timer_frames.append(pd.DataFrame([{
            ...
            "model_name": "greedy_class_weight",
            "n_models": get_greedy_total_models_from_coefficients(scenario_name),
...
def train_or_load_resampling_strategy(scenario_name, config, strategy_name, timer_frames):
    ...
    if coefficients_are_compatible_for_strategy(scenario_name, strategy_name, feature_cols):
        ...
        timer_frames.append(pd.DataFrame([{
            ...
            "model_name": strategy_name,
            "n_models": get_total_single_strategy_train_models(),
            "total_seconds": np.nan,
            "total_time_readable": "coeficientes reutilizados",
            "skipped_because_existing_outputs": True,
        }]))
...
def pretty_split_label(split_name):
...
def pretty_scenario_label(scenario_name):
    text = str(scenario_name).strip()
    risk_prefix = f"{REAL_SCENARIO_PREFIX}_suicide_risk"
    if text.startswith(risk_prefix):
        risk_number = text.replace(risk_prefix, "")
        return f"Real 2 · Risk {risk_number}"
    text = text.replace("_", " ")
...
def format_metric_label_for_plot(metric, include_split=True):
    ...
    if parts and parts[-1] in {"mean", "std"}:
        stat_token = parts.pop()
        stat_label = "Mean" if stat_token == "mean" else "STD"

    split_label = ""
    if parts and parts[0] in {"val", "test"}:
        split_label = pretty_split_label(parts.pop(0))
    ...
    labels = []
    if include_split and split_label:
...
def format_metric_axis_label_for_plot(metric):
    return format_metric_label_for_plot(metric, include_split=True)
...
def make_bar_labels(metric_mean, metric_std, c0_value, c1_value, model_name=None, c0_std=np.nan, c1_std=np.nan):
    ...
    Se separan en dos bloques verticales:
    - bloque superior: Mean y STD;
    - bloque inferior: c0 y c1.
...
def save_bar_comparison_plots(agg_df, split_name):
    ...
    for metric in metrics:
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([pretty_scenario_label(s) for s in scenario_order], rotation=0, ha="center")
        ax.set_xlabel("Escenario", labelpad=10)
        ax.set_ylabel(format_metric_axis_label_for_plot(metric))
        ax.set_title(f"{pretty_split_label(split_name)} · Comparación de estrategias · {format_metric_label_for_plot(metric, include_split=False)}", fontsize=13)
...
def _draw_confusion_matrix_axis(ax, group, model_name, split_name):
    ...
    ax.set_title(
        f"{MODEL_LABELS.get(model_name, model_name)}\n{pretty_split_label(split_name)} · {n_seeds} seeds · n={len(group)}",
        fontsize=9.5,
        pad=7,
    )
    ...
    for scenario in scenario_order:
        ...
        if "equal_weights" in model_order:
            ...
            if not equal_df.empty:
                ...
                fig.suptitle(
                    f"{pretty_split_label(split_name)} · Matriz de confusión referencia 1:1 · {pretty_scenario_label(scenario)}",
                    fontsize=13.5,
                    y=0.985,
                )
        ...
        fig.suptitle(
            f"{pretty_split_label(split_name)} · Matrices de confusión · Estrategias de desbalanceo · {pretty_scenario_label(scenario)}",
            fontsize=15,
            y=0.985,
        )
...
def save_boxplot_plots(raw_df, split_name):
    ...
    for metric in metrics:
        ...
        for s_idx, scenario in enumerate(scenario_order):
            ...
            xticklabels.append(str(scenario))
        ...
        ax.set_xlabel("Escenario", labelpad=10)
        ax.set_ylabel(format_metric_axis_label_for_plot(metric))
        ax.set_title(f"{pretty_split_label(split_name)} · Caja y bigotes · {format_metric_label_for_plot(metric, include_split=False)}", fontsize=13)
        ax.grid(axis="y", linestyle="--", alpha=0.25)
...
def save_pr_roc_curve_panels(predictions_df, results_df, split_name):
    ...
    for scenario in scenario_order:
        ...
        fig.suptitle(
            f"TEST · Curvas PR y ROC comparativas · {pretty_scenario_label(scenario)}",
            fontsize=15,
            fontweight="bold",
            y=0.975,
        )
...
def extract_model_count_from_timer_line(line, alias_list):
    """
    Extrae el primer entero situado después del nombre de la estrategia en una
    fila de tiempos. Si el código base ya incluye la columna de modelos,
    permite recuperarla directamente desde timers_execution.txt.
    """
    line = str(line)
    after_alias = ""
    for alias in alias_list:
        pos = line.find(alias)
        if pos >= 0:
            after_alias = line[pos + len(alias):]
            break

    if not after_alias:
        return np.nan

    matches = re.finditer(r"(?<![\d.,])(\d{1,12})(?![\d.,])", after_alias)
...
def save_visual_table_by_scenario(table_df, output_path, title):
    ...
    for ax_idx, ax in enumerate(axes):
        ...
        if scenario_df.empty:
            ax.set_title(f"Escenario: {pretty_scenario_label(scenario_name)}", fontsize=13.5, fontweight="bold", pad=10)
            continue

        ax.set_title(f"Escenario: {pretty_scenario_label(scenario_name)}", fontsize=16.5, fontweight="bold", pad=12)
...
def parse_base_timer_totals():
    """
    Recupera los tiempos reales de las estrategias base desde timers_execution.txt.

    Soporta dos formatos:
    - filas donde aparece el escenario en la propia fila;
    - bloques por escenario, como en Real 2, donde primero aparece
      "ESCENARIO: ..." y debajo las filas de cada estrategia.
    """
    ...
    aliases = {
        "best_val_f1_grid": ["Weighted grid / Best F1", "Weighted Best F1", "Manual Best F1", "Best F1"],
        "equal_weights": ["Equal weights"],
        "sklearn_balanced": ["Sklearn balanced"],
    }
    ...
    def make_empty_row(scenario_name, model_name):
        return {
            "escenario": scenario_name,
    ...
    def find_lines_for_scenario(scenario_name):
        ...
        for line in lines:
            ...
            if upper.startswith("ESCENARIO:"):
                current = stripped.split(":", 1)[1].strip()
                inside = current.lower() == scenario_name.lower()
                continue
            if inside:
                if set(stripped) == {"#"} and scenario_lines:
                    break
        ...
        # Respaldo para formatos donde el escenario aparece en cada fila.
        return [line.strip() for line in section.splitlines() if scenario_name in line.strip()]

    for scenario_name in get_scenarios().keys():
        scenario_lines = find_lines_for_scenario(scenario_name)

        for model_name, alias_list in aliases.items():
            base_row = make_empty_row(scenario_name, model_name)
            matching_lines = [
                line for line in scenario_lines
                if line and any(alias in line for alias in alias_list)
            ]

            if not matching_lines:
                # Último respaldo: buscar en todo el texto.
                matching_lines = [
                    line.strip() for line in text.splitlines()
                    if str(scenario_name) in line.strip() and any(alias in line.strip() for alias in alias_list)
                ]
            ...
            models_from_line = extract_model_count_from_timer_line(line, alias_list)
            if not pd.isna(models_from_line):
                base_row["modelos_entrenados"] = int(models_from_line)
            ...
            if pd.isna(base_row["modelos_entrenados"]):
                expected_models = get_expected_base_models_for_strategy(model_name)
                if not pd.isna(expected_models):
                    base_row["modelos_entrenados"] = int(expected_models)
...
def save_timer_summary(timer_frames, total_start):
    ...
    if not base_time_df.empty:
        ...
        for model_name in BASE_MODEL_NAMES:
            expected_models = get_expected_base_models_for_strategy(model_name)
            if pd.isna(expected_models):
                continue
            mask = (
                (base_time_df["estrategia"].astype(str) == str(model_name))
                & (
                    base_time_df["modelos_entrenados"].isna()
                    | (pd.to_numeric(base_time_df["modelos_entrenados"], errors="coerce").fillna(0) == 0)
                )
            )
            base_time_df.loc[mask, "modelos_entrenados"] = int(expected_models)
    ...
    for scenario_name in get_scenarios().keys():
        for strategy_name in TRAINED_COMPARISON_MODEL_NAMES:
            ...
            if strategy_name == "greedy_class_weight":
                models_fallback = get_greedy_total_models_from_coefficients(scenario_name)
                if models_fallback == 0:
                    models_fallback = np.nan
            else:
                models_fallback = get_total_single_strategy_train_models()
            ...
            new_rows.append({
                ...
                "estado": estado,
                "modelos_entrenados": get_wall_n_models(train_stage, scenario_name, strategy_name, fallback=models_fallback),
...
def run_comparison():
    ...
    print("\n" + "#" * 100)
    print("COMPARACIÓN FINAL DE ESTRATEGIAS · DATASET REAL 2")
    print("#" * 100)
    print(f"Salida: {OUTPUT_DIR.resolve()}")
    print("Estrategias nuevas: WBCE Heurística, RandomOverSampler, SMOTE y RandomUnderSampler.")
    print("Todas usan las mismas 20 seeds y el remuestreo se aplica solo al TRAIN.\n")
    ...
    metadata = {
        "description": "Comparación final de WBCE frente a WBCE Heurística y estrategias clásicas de remuestreo para Dataset Real 2.",
        "comparison_run_signature": COMPARISON_RUN_SIGNATURE,
        "base_output_dir": str(BASE_OUTPUT_DIR),
        "base_dataset": DATA_FILE_NAME,
        "targets": TARGET_COLS,
        "output_dir": str(OUTPUT_DIR),
        "seeds": SEEDS,
        "greedy_strategies": GREEDY_MODEL_NAMES,
        "resampling_strategies": RESAMPLING_MODEL_NAMES,
        "trained_comparison_strategies": TRAINED_COMPARISON_MODEL_NAMES,
